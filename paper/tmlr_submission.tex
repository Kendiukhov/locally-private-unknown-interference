...
% Official TMLR submission mode: no [accepted] or [preprint] option.
\usepackage{tmlr}
...
\hypersetup{
  ...
  pdftitle={Locally Private Causal Inference with Unknown Within-Block Interference},
  pdfsubject={Anonymous submission to Transactions on Machine Learning Research},
  pdfkeywords={local differential privacy, causal inference, interference, randomized experiments}
}
...
Design-Based Effects, DoI Models, and Minimax Limits\thanks{Large-language-model tools were
used to assist with literature search, code generation, numerical verification, and manuscript
drafting. The submitting authors assume responsibility for all content.}}
\author{Anonymous Authors}
\date{}

% These fields are used only after acceptance; anonymous values keep this source camera-ready.
\def\month{MM}
\def\year{YYYY}
...
The \href{https://anonymous.4open.science/r/locally-private-unknown-interference-7BE9}{anonymized
artifact repository} contains the code, configurations, raw and summarized results, figure
scripts, tests, seeds, environment specification, checksums, and reference-verification ledger.
All empirical data are synthetic; no human-subject, proprietary, or externally collected
dataset was used.

\subsubsection*{Broader Impact Statement}

This work may support experimental analysis with less revealing outcome reports. The guarantee
protects only each bounded outcome message; public assignments, block membership, covariates,
network structure, and inference from correlated peers remain outside it. Calling outcome-only
LDP network or group privacy could provide false assurance, and cross-block interference could
invalidate uncertainty. We mitigate by stating assumptions, reporting diagnostics, and
separating design claims from model-dependent regression. Deployments should assess consent,
composition, public-metadata sensitivity, clipping, and block validity.
